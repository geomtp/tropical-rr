...
$\opn{td}(B) \in \mb{H}^{\bullet}(B;\mcal{F}_{\Z, B}^{\bullet})$.
 Let $s$ be an admissible (weak-)smooth Cartier divisor satisfying an admissible condition 
...
without differential for admissible divisors 
later.
...
\begin{proposition}
Let $(s',s)$ be an admissible smooth Cartier pair, 
...
\subsection{Local Morse data for admissible divisors}
...
\begin{proof}
Obviously, 
$\opn{dim}_{\mathbb{R}}\opn{Lin}(|\Sigma|,x)\leq 1$.
If $\opn{dim}_{\mathbb{R}}\opn{Lin}(|\Sigma |,x)=1$, 
then $|\Sigma | \simeq \mathbb{R}$. 
From now on, we assume $\dim_{\mathbb{R}}\opn{Lin}(|\Sigma|,x)=0$.
...
As a colloary, we get the following isomorphism 
for the $n$-th symmetric product $S^{n}X$ of 
rational polyhedral spaces even though 
the $S^{n}X$ is not a rational polyhedral space. 

\begin{corollary}
Let $k$ be a field and $X$ a compact rational 
polyhedral space satisfying \cref{condition-Rn} 
and $s$ an admissible Cartier divisor 
such that $\opn{LMD}^{\bullet}_k(X;s)$ is bounded, 
finitely generated and 
satisfies \cite[Assumption 1.1.1]{MR2031639} locally.
Let $\opn{Sym}(s)$ be the $n$-th symmetric product of 
$s$ defined like $s\boxtimes s'$ and 
$\opn{LMD}^{\bullet}_{k}(S^{n}X;\opn{Sym}(s))$
the local Morse data of $\opn{Sym}(s)$ defined like $s$.
Then,
\begin{align}
\chi(\opn{LMD}_k^{\bullet}(S^{n}X;\opn{Sym}(s)))=
\binom{n+\chi(\opn{LMD}_k^{\bullet}(X;s))-1
}{n}.
\end{align}
...
(see also \cite[Proposition 1.3.1]{MR2031639}). 
From construction,
we can apply Macdonald formula \cite{MR143204} for 
$\opn{LMD}_k^{\bullet}(S^{n}X;\opn{Sym}(s))$.
\end{proof}
