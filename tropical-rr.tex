...
Many examples of tropical manifolds (or tropical spaces) 
...
are detailed in \cite{MR3287221}. 
Algebraic varieties and tropical varieties are 
...
on $U_i$. The set 
$\{(U_i,f_i)\}_{i\in I}$ is 
a \emph{smooth Cartier local data} for $s$ if 
$s|_{U_i}=(f_i)^{\mathrm{sm}}$ for all 
$i\in I$. The set $\{(U_i,f_i)\}_{i\in I}$ is a
\emph{smooth Cartier local data} if 
