...
The following condition is useful.
...
$\varepsilon >0$ at the origin and 
$r(x)\deq \sum_{i=1}^{n}x_i^{2}-\varepsilon^{2}$.
Then, from \cite[Proposition 5.3.3]{MR1299726}
\begin{align}
\opn{SS}((k_{U})_{\bar{B}(0,\varepsilon)})=
\{(x;\lambda dr(x))\in T^*U\mid x\in \bar{B}(0,\varepsilon), 
...
\opn{SS}((k_{U})_{\bar{B}(0,\varepsilon)}))\cap S(0,\varepsilon)}
...
\begin{align}
\opn{ind}_x^{k}(f)=
\sum_{x\in \opn{Crit}(\tilde{f})\cap K}
\opn{ind}^{\opn{PH}}_x(\opn{grad}(\tilde{f}))=
\opn{ind}^{\opn{PH}}_x(\opn{grad}(f)).
\end{align}

We note
$\opn{LMD}^{\bullet}(k_M,\tilde{f})$ is isomorphic
to the graded module of a $k$-valued Morse complex
of $\tilde{f}$.
\end{example}
...
The following example is a typical example of
...
for $S$. Instead of this fibration, 
...
(\mathcal{F}_{S}^{1})_x\otimes_{\mathbb{Z}}\mathbb{R}
...
The Lagrangian section associated with $s$ is the following
...
\begin{align}
\check{s}\colon C\to \check{X}_0 (C); x\mapsto (x,\opn{pr}_{C,x}(df_{i}(x)))
\end{align}
