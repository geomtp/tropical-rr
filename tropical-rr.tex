...
(\opn{LC}_x S/\opn{Lin}(\opn{LC}_x S))$, and thus
\begin{align}
...
(\mathcal{F}_{S}^{1})_x $ if 
$\opn{Lin}(\opn{LC}_x S)=\{0\}$.
From now on, we assume $\opn{Lin}(\opn{LC}_x S)=\{0\}$.
Next, we prove the following inclusion:
\begin{align}
  (\bigcap_{y \in (\opn{LC}_xS)_{\opn{reg}}} 
...
for simplicity.
We can calculate the degree of proper morphism of equi-dimensional 
tropical manifolds by 
