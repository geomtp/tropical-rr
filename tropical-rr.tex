...
\subsection{SYZ conjecture and Lagrangian sections}
...
fibration is comes from integral affine manifolds.
An $n$-dimensional 
...
The following proposition are important for 
...
a \emph{Lagrangian section}, a section $g$ of 
$\check{f}_{B}$ such that $\opn{Im}(g)$ is 
a Lagrangian submanifold of 
$\check{X}(B)$. Besides, 
$g$ can be considered as an object
$\mathscr{L}_{s}$ of 
$\catn{fuk}(\check{X}(B))$ by a standard choice of 
additional data.
...
integral affine manifold $B$ has a $C^{\infty}$-Cartier
...
$\opn{td}(B) \in \mb{H}^{\bullet}(B;\mcal{F}_{\Z, B}^{\bullet})$.
 Let $s$ be an permissible (weak-)$C^{\infty}$-Cartier divisor satisfying an permissible condition 
...
The condition of permissibility is that every cotangent vector 
...
(without differential) for some good $C^{\infty}$-Cartier 
