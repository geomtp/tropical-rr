...
is \emph{not} an invariant of $\mb{T}$-modules
\myfootnote{In fact, 
...
is not studied yet.
\myfootnote{The nerve of a proto-exact category
...
(e.g. \cite[p.50]{}) and thus we can calculate
...
\begin{itemize}
\item Any topological space is locally compact and locally
...
\begin{align}
\Gamma_Zf_*\mcal{G}\simeq f_*\Gamma_{f^{-1}(Z)}\mcal{G}.
\end{align}
...
Let $f\colon X\to \R$ be a $C^{1}$-function on a 
$C^{1}$-manifold $X$ and has 
...
\end{align}
\end{Def}
We follow the above notation from \cite[p.271]{MR2031639}.
\begin{Rmk}
...
of $X$ (see for example \cite[p.199]{hatcherAlgebraicTopology2002a}).
We note $\tilde{H}^{-1}(\emp;A)=A$ and $\tilde{H}^{i}(\emp;A)=0$
for any $i\in \N$ from definition.

\begin{proof}
Let $i\colon S\to X$ and
$j\colon \{f<f(x)\}\to X$ be the inclusion maps.
...
\end{align}
From the definition of higher direct image, we have
\begin{align}
  (R^{n} (j \circ i')_* A_{S\cap \{f<f(x)\}})_x\simeq 
\varinjlim_{U\in \opn{Op}(X)\cap \opn{Nbh}(x)} H^{n}(S\cap \{f<f(x)\}\cap U;A)
\end{align}
\end{proof}

\cref{prop-local-morse-data}
