...
affine manifold $B$
with a Hessian form) and $s=\{(U_i,f_i)\}_{i\in I}$ a admissible 
...
defined from the differental $dq$ of a quadratic polynomial
$q$ function on the universal cover $\tilde{T}$ of $T$.
...
of the linear part of $dq$.

Every linearlity equivalent 
...
the homology class in $H^{\bullet}(\check{X}(B_0);\Z)$.

If $p^{*}(s)$ intersect to the zero section of 

$\check{\pi}_{B_0}$ transversaly,

$\opn{MF}^{\bullet}(B_0,p^{*}(s))\otimes_{\Z}\Lambda_{\opn{nov}}=
\opn{CF}^{\bullet}(L_{p^{*}(s)},L_{0})$.

In particular, $\chi(\opn{MF}^{\bullet}(B_0,p^{*}(s)))$ is 
equal to the intersection number 
$\sharp ([L_{p^{*}(s)}]\cap [L_0])$ up to signature.
