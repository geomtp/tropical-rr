...
D^{b}(A_Y)$. Then,
\begin{align}
\opn{SS}(f^{-1}\mathcal{F})=
{}^{t}f'(f_\pi^{-1}(\opn{SS}(\mathcal{F})))
\end{align}
...
(\opn{SS}(A_{\opn{LC}_x S\subset T_x S})_x)+\mathcal{F}_{\mathbb{Z},S,x}^{1})
\setminus (\mathcal{F}_{\mathbb{Z},S,x}^{1})^{\vee}.
\end{align}
\end{condition}
%TODO: 表記の理由を何とかする。
...
a local smooth Cartier data. In this paper, we
...
$\mathcal{F}_{Q,X}^{\bullet}$ has a canonical dga structure, 
...
The (tropical) degree of $f$ is the integer $m$
such that $f_!([X])=m[Y]$.
\end{definition}
...
\begin{align}
\chi(\opn{MF}^{\bullet}(B';f^{*}(s)))=\opn{deg}(f)
\end{align}
\end{proposition}
\begin{proof}
