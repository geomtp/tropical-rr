...
every nondepraved function satisfies Whitney b-condition
for $(\{f(x)=0\},x)$.
...
if $(x,df(x))\notin (\opn{span}_{\R}(\opn{SS}(A_S)_x)\setminus
T^{*}_{M_{\R}} M_{\R})+M^{\vee}$.
\end{Cond}
...
Take a projection $M\to M/(\opn{span}_{\R}(\opn{SS}(A_S)_x))\cap M$
Since $S$ is a rational polyhedral space and thus 
$\opn{SS}(\Z_S)$ also 
Then, $df(x)\in M$ if and only if 
$df(x)\in M/(\opn{span}_{\R}(\opn{SS}(A_S)_x))\cap M$
...
Let $S$ be a compact rational polyhedral space such that any atlas is 
...
\begin{Conj}
Let $S$ be a tropical manifold such that any atlas is in for some 
...
\opn{Im} (\delta : H^{0}(B,\mcal{D}iv_{B})\to 
...
Thus, the commutative diagram is written like this 
