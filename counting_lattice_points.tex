...
For $q \colon B' \to B$ be a tropical \`etale map of $B$ of 
(topological) degree 
$\opn{deg}_{\opn{top}}(q)$,
\begin{align}
...
not a endomorphism of $X$ (over $\mathbb{T}$) but 
there exists the following commutative diagram:
\begin{equation}
\begin{tikzcd}
...
\begin{remark}
The condition $\pi_2(B_0)=0$ may seem to be technical, but 
...
Every integral affine manifold is a double cover of special integral 
affine manifold and thus the universal cover of closed integral affine 
...
Let $f\colon B'\to B$ be a finite tropical \'etale covering of a compact integral affine 
manifold $B$ of dimension $n$. Then,

\begin{align}
\chi(\opn{MF}^{\bullet}(B';f^{*}(s)))=
\opn{deg}(f)\chi(\opn{MF}^{\bullet}(B;s)).
\end{align}
\end{proposition}
\begin{proof}
