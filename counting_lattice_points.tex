...
One of remarkable feature is that 
the rank of linear system of tropical curves is
not the invariant of $\mb{T}$-modules 
\footnote{}. One of 
...
\cite[Example 7.2.3]{} and there exists the
stable hull of it}. 
...
$df(x)=\frac{e^{x}}{1+e^{x}}$ 
\footnote{softmax functions are appreared 
...
The follwoing question is not formulated as a mathematical 
...
\begin{Ques}[{Tropical Mirror Riemann-Roch Problem}]
Let $B$ be a compact tropical manifold and $\mcal{L}$
be a line bundle on it. Let $s$ be a transversal polite 
smooth Cartier data which represents of $\mcal{L}$.
...
The condition of politeness is that every cotagent vector 
is not in planes defined by the span of microsupport of
...
\section{The cohomological local Morse data}
