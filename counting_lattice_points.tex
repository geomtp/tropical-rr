...
H^{-q}R\opn{Hom}(\Omega_X^{p},\upomega_X^{\bullet})\simeq 
...
H^{i,j}(X) \times 
H_{p,q}^{\opn{BM}}(X)\to 
H_{p-i,q-j}^{\opn{BM}}(X);(c,\alpha) \mapsto c\frown \alpha$.
...
function with value one and this corresponds to 
the identity morphism 
$\opn{id}:\upomega_X^{\bullet} \to \upomega_X^{\bullet}$.
...
the choice of the orientation like complex manifolds.   
\end{remark}
...
The (tropical) degree of $f$ is 
$f_*\colon H^{\mathbb{BM}}_{n,n}(X)
\to H^{\mathbb{BM}}_{n,n}(Y)$.
\end{definition}

\begin{example}
Let $f\colon X\to Y$ be a morphism of 
rational polyhedral spaces such that $f$ is a 
covering map and the associated map $df_{*,x}\colon 
T_{x}^{\mathbb{Z}} X\to T_{f(x)}^{\mathbb{Z}}X$ is 
isomorphic, then $f_*[X]=[Y]$.
\end{example}
