...
we can define the rank $\opn{rank}_A M$ of 
...
$\chi(M^{\bullet})\deq \sum_{i\in \Z}(-1)^{i}\opn{rank}_A M^{i}$.
If $(M^{\bullet},d)$ is a bounded chain complex of finitely 
generated $A$-module, then $\chi(M^{\bullet})=\chi(H^{\bullet}(M))$.
...
stratified Morse theory (e.g. \cite[Part 1.2.1]{}). 
...
=\opn{ind}_{\opn{PH}}(\nabla f,x).
\end{align}
Here, $\opn{ind}_{\opn{PH}}(\nabla f,x)$ is the 
Poincare--Hopf index of the gradient of $f$.
...
Pincar\'e--Hopf theorem for some polyhedral spaces.
\end{Eg}
