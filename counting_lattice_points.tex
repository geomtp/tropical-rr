...
\begin{proposition}
Let $C$ be a connected tropical curve with no 
1-valent vertex which is not homeomorphic to a circle 
and $s$ be a smooth Cartier divisor on $C$. 
For any open subset $U$ of $C_0$, there exists a $(0,0)$-superform
$f\colon U \to \mathbb{R}$ such that
$(f)^{\opn{sm}}=s|_{U}$.
\end{proposition}
\begin{proof}
Since $\mathcal{O}^{\times}_{C_0}$ is a locally constant 
sheaf on $C_0$ and $C_0$ is a disjoint union of 
open intervals, 
$H^{1}(C_0;\mathcal{O}^{\times}_{C_0})=0$ and thus
$(\cdot)^{\mathrm{sm}}$ is surjective.
\end{proof}

\begin{definition}[{Edge data}]

\end{definition}

\begin{definition}
Let $C$ be a connected compact tropical curve with
no 1-valent vertex and $p\in C$. 
A \emph{bump function} of $p$
is a $(0,0)$-superform such that
\begin{enumerate}
\item $C\setminus \opn{supp}(f)$ is contractible.
\item $\opn{Crit}(f)\setminus \opn{supp}(f)=\{p\}$.
\item $f$ is concave at $p$.
\end{enumerate}
\end{definition}
...
\begin{proposition}
Let $s$ be an admissible smooth Cartier divisor. 
Then, there exists a simple admissible
smooth Cartier divisor such that
$\chi(\opn{MF}^{\bullet}(C;s))=
\chi(\opn{MF}^{\bullet}(C;s'))$
and $\opn{rot}(s)=\opn{rot}(s')$.
\end{proposition}

\begin{proof}
We can create $s'$ by adding some good 
bump functions to $s$.
\end{proof}
...
\begin{definition}[{Standard function}]
Let $C$ be a connected compact tropical curve with
no 1-valent vertex which is not homeomorphic to 
a circle and $p\in C$.
A \emph{bump function} on $C$ is a
$(0,0)$-superform such that:
\begin{enumerate}
\item $\sharp (\opn{Crit}(f))<\infty$,
\item $C\setminus C_0\subset \opn{Crit}(f)$,
\item $f$ is convex at all $p\in C\setminus C_0$,
\item $\sharp (\opn{Crit}(f)\cap e)=1$ for all connected 
      component $e$ of $C_0$.
\end{enumerate}
\end{definition}

\begin{proposition}
Let $C$ be a compact tropical curve with no 1-valent
vertex which is not homeomorphic to a circle
 and $f$ a standard function on 
$C$. Then, $\chi(\opn{MF}(C;(f)^{\mathrm{sm}}))=\chi_{\mathrm{top}}(C)$.
\end{proposition}
\begin{proof}
If $C$ is homeomorphic to a circle, 
