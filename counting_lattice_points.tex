...
Lagranigian submanifolds for line bundles on 
tropical spaces?  
\end{question}
...
tropical line bundles on a rational polyhedral 
spaces is parametrized by 
...
and thus $H^{1}(B,\mcal{O}^{\times}_B)\simeq  
\opn{Im} (\delta \colon H^{0}(B,\mcal{M}_B^{\times}/\mathcal{O}^{\times}_B)\to 
H^{1}(B, \mcal{O}^{\times}_B))$ \cite[Lemma 4.5, Proposition 4.6]{MR3894860}.
The group $H^{1}(B, \mcal{O}^{\times}_B))$ is called 
the \emph{Picard group} of $B$.

We didn't define a analog for line bundles of  
on rational polyhedral spaces directly.

From now on, we define the admissible condition for
\emph{smooth Cartier divisors}.



We will define an analog of Floer complex 
...
First, we introduce the \emph{admissibility condition}
...
The \emph{primncipal smooth divisor} of a $(0,0)$-superform 
...
\begin{remark} \label{rmk: cartier module}
...
\begin{example} \label{eg: kodaira}
...
Let $S$ be a compact rational polyhedral space such that any atlas is in for some 
...
\begin{definition}[Smooth Cartier data]
