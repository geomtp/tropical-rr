...
of SYZ conjecture and HMS conjecture for 
Calabi--Yau manifolds.  
%TODO: SYZとは

\ind{Strominger-Yau-Zaslow conjecture}{Strominger-Yau-
Zaslow conjecture} or 
\ind{SYZ conjecture}{SYZ conjecture} is about 
...
\opn{Fuk}(\check{X})\to D^{b}(\opn{Coh}(X))$
...
should map to a line bundle on $X(B_0)$. 
...
\begin{align}
\label{equation-MRR}
\chi (\opn{HF}^{\bullet}(B_0;s))=
\int_{X(B_0)}\opn{ch}(\mathcal{L}_s)\opn{td}(X(B_0))
\end{align}
...
From definition of Floer cohomology and 
...
=(-1)^{n} \chi(\opn{HF}^{\bullet}(\mathscr{L}_2,\mathscr{L}_1)).
\end{align}
This is also a special result of Poincar\'e duality for 
$\mathbb{Z}/N$-graded Floer cohomology, 
see \cite[2f]{seidelGradedLagrangianSubmanifolds2000}.
On othe other hand, let $X$ be a complete smooth 
...
Obviously, for many complex manifolds $\check{X}$
there is no $X$ satisfying the condition.
...
\begin{align}
\chi(\opn{MF}^{\bullet}(\Z_S,f,x))=
(-1)^{\dim S}\chi(\opn{MF}^{\bullet}(\Z_S,-f,x)).
\end{align}
...
Suppose $\dim M_{{\mathbb{R}}}=\dim 
\mathcal{F}_{S,x}^{1}$. In this case,
...
(\opn{SS}(A_S)_x)=\dim \mathbf{F}_{S,x}$,
$f$ is admissible if and only if 
$df(x)\in M^{\vee}$. Therefore, admissibility
condition is not an open condition.
If $\dim_{\mathbb{R}} \opn{span}_{{\mathbb{R}}}
(\opn{SS}(A_S)_x)<\dim \mathbf{F}_{S,x}$, the 
...
(\mathcal{F}_{S,x}^{\Z}/(\opn{span}_{{\mathbb{R}}}
(\opn{SS}(A_S)_x))\cap \mathcal{F}_{S,x}^{\Z})\otimes_{\Z} {\mathbb{R}}/\Z.
\end{align}
...
$\opn{MF}^{\bullet}(S;s,A)$ as the cohomology of $s$,
but just
a graded module 
...
It is important that $\opn{MF}^{\bullet}(S;s,A)$ is
...
For $q \colon B' \to B$ be a covering map of $B$ of 
(topological) degree 
$\opn{deg}_{\opn{top}}(q)$,
\begin{align}
...
\cref{condition-Rn} and has the Todd class $\opn{td}(B)$ 
of $B$ (see \cref{remark-todd-class}).
Let $s$ be an admissible smooth Cartier divisor such that
$s_0\cap s$ is finite. Then, 
\begin{align}
\chi(\opn{MF}^{\bullet}(B;s))=\int_{B}\opn{ch}(s)\opn{td}(B).
\end{align}
\end{conjecture}
...
invariant for line bundles for general compact rational 
...
the following condition,
but we expect that the condition is unnecessary
in general.
...
\opn{Lin}(S,x)^{\bot}(\simeq 
(\mathcal{W}_{1,S}^{\mathbb{R}})_{x}^{\bot}).
\end{align}
where $(\mathcal{W}_{1,S}^{\mathbb{R}})_{x}$ is the stalk 
of the sheaf of (real) wave tangent vector spaces of $S$ at $x$
(see 
\cite[1.3]{mikhalkinTropicalEigenwaveIntermediate2014a} and 
\cite[Definition 2.16]{yamamotoTropicalContractionsIntegral2021}).
\end{condition}
See also \cite[Example 3.3]{mikhalkinTropicalEigenwaveIntermediate2014a} and 
\cite[Remark 2.21]{yamamotoTropicalContractionsIntegral2021} for the wave tangent space
 of $1$-dimensional local cones.

%TODO: eigenwaveとの諸々の文献補填
...
\cite[]{MR1299726}.
From elemental properties of 
adjoint functors we have
\begin{align}
...
$f^{-1}(\mathcal{F})\otimes \upomega_{Y/X}\to 
f^{!}(\mathcal{F})$.
...
such that $s_0\cap s=C\setminus C_0$. Then,
\begin{align}
...
\begin{remark}
%TODO: admissible な関数のなす層にして理論を展開
%したらうまくいくことをかく。
%TODO: $(\mathcal{O}_B^{\times})^{\opn{adm}}$
\end{remark}
...
%TODO: 適切な箇所で See \cref{remark-non-admissible-divisor}
...
\begin{question}
  Can we generalize the correspondence
\ref{equation-local-RR} for pants decomposition of
  complex algebraic hypersurfaces in \cite{MR2079993} and line bundles?
\end{question}

We also expect that we can generalize the 
above correspondence for more general the complement of
hyperplane arrangements and flat bundles on it.


%TODO: amissibleのなす層がアーベル群のなす層では
%Torsorとしての性質のを満たすことを述べる。
...
Here, $\mcal{F}_{\Z, B}^{0}={\mathbb{Z}}_B$. 
...
for a purely $n$-dimensional tropical manifold
\cite[Theorem 6.2]{gross2019sheaftheoretic}.

The The $(p,q)$-th Borel-Moore homology of $X$ is
\begin{align}
H^{\opn{BM}}_{p,q}(X;\Z)\deq 
H^{-q}R\opn{Hom}(\Omega_X^{p},\upomega_X^{\bullet})=
\opn{Hom}_{D^{b}(\mathbb{Z}_X)}(\Omega_X^{p}[q],\upomega_X^{\bullet}).
\end{align}
$H_{0,q}^{\opn{BM}}(X;\Z)=H^{-q}R\opn{Hom}(A_X,\upomega_X)$ is equal to the classical Borel-Moore homology
...
\begin{align}
H_{p,q}^{\mathrm{BM}}(X)\simeq H^{n-p,n-q}(X).
\end{align}
...
and thus $H_{0,0}^{\opn{BM}}(B;\Z)\simeq 
H^{n,n}(B;\Z)=H^{n}(B;\FBZ^{n})\simeq \Z$ if 
$B$ is compact and path-connected.

\begin{align}
H^{\opn{BM}}_{n,n}(X,\Z)&\simeq \opn{Hom}_{\opn{D}^{b}(\Z_X)}(\upomega_X,\upomega_X)\simeq
 \opn{Hom}_{\opn{D}^{b}(\Z)}(R\Gamma_c\upomega_X,\Z), \\ 
H^{n,n}(X;\mathbb{Z})&\simeq 
\opn{Hom}_{\opn{D}^{b}(\Z_X)}(\mathbb{Z}_X,\upomega_X^{\bullet})
\simeq  \opn{Hom}_{\opn{D}^{b}(\Z)}(R\Gamma_c\Z_X,\Z).
\end{align}
...
\begin{definition}[{degree of proper morphism}]
...
Therefore, $H^{n,n}(X;\Z)\xto{\cdot \frown [X]} H_{0,0}^{\opn{BM}}(X;\Z)
...
\begin{Not}
  ...
  Therefore, we write 
$\AffS_{\Z,B_0}\deq \mcal{O}^{\times}_{B_0}$, 
$\mcal{T}_{\Z,B_0}\deq \mcal{F}_{\Z,B_0}^{1}$ in this section for integral affine manifolds. We also use this notation for integral affine manifold with singularities.
\end{Not}
...
and thus every pair $(L_1,L_2)$ of Lagrangian section
of $B_0$ has a ($\mathbb{Z}/2\mathbb{Z}$-graded)
Floer cohomology $\opn{HF}^{\bullet}(L_1,L_2)$. 
Its Euler number is equal to the intersection of 
$L_1$ and $L_2$ (up to signature).
Besides,
if $B_0$ is a compact integral affine 
Hessian manifold and has a spin structure, every 
Lagrangian section has a canonical structure 
as a object of the Fukaya category of 
$\check{X}(B_0)$ 
\cite[5.2]{MR1882331}.

\begin{remark}
When $s=\vep df$ for a sufficiently small $\vep >0$ and 
a Morse function $f$ on $B$, $\chi(B,s)=\chi_{\opn{top}}(B)=0$ is 
...
$f_*\colon H_{n,n}(B;\mathbb{Z})\to 
 H_{n,n}(B';\mathbb{Z})$
\end{definition}

\begin{definition}
Let $B,B'$ be an integral affine manifold. 
An integral affine map $f\colon B\to B'$ is a \emph{\'etale} if
$f$ is locally isomorphic as integral affine manifolds.
\end{definition}
...
complex manifolds.
When $f$ is not an \'etale in our sense,
$\opn{deg}_{\opn{top}}(f)\ne \opn{deg}_{\opn{top}}(X(f))$ in general.
...
\begin{proof}
From definition of trace map
$\int_B=\int_{B'}\circ \int_{B/B'}$.


\begin{align}
\opn{Hom}_{D^{b}(\mathbb{Z}_B)}(
\upomega_B^{\bullet},\upomega_B^{\bullet})
\simeq \opn{Hom}_{D^{b}(\mathbb{Z}_{B'})}
(Rf_! \upomega_B^{\bullet},\upomega_{B'}^{\bullet}) \to
\opn{Hom}_{D^{b}(\mathbb{Z}_{B'})}
(\upomega_{B'}^{\bullet},\upomega_{B'}^{\bullet})
\end{align}
...
\emph{numerical quantization problem}
 in \cite{1999math......2027T}.
...
The precise setting of the conjecture is 
in \cite[Conjecture 1.6]{MR3525095}.

We stress that the $\sharp \mathrm{BS}=\mathrm{RR}$ is 
true for closed integral affine manifold 
such that $X(B_0)$ is not K\"ahlerian, i.e., 
not Calabi-Yau type. In this case,
