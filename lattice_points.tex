...
In this paper, we want to study these analogue from the view point of tropical geometry and symplectic geometry.
...
\begin{Note}[Guide for reading this paper]
  ...
  and other topics, we cannot avoid to introduce various
...
\begin{Not}
  ...
  For every continous function $f\colon X\to \R$,
...
The following graded module is the most important notion
for this paper which is naturally appeared from sheaf theory.

\begin{Not}
  The notion and symbols for sheaf theory follows from \cite{MR1299726}.
...
and the cochain complex
$\opn{LMD}(\mcal{F},f,v)\deq R\Gamma_{\{f\geq f(v)\}}(\mcal{F})_v$
is called the sheaf theoretic counterpart of the (cohomological)
"local Morse datum" in \cite[p.271]{MR2031639}.
...
If $A$ is a PID, then there exists
the Euler characteristic map
$\chi:K_0(\opn{D}^{b}(\opn{mod}(A)))\simto \Z;
  [C^{\bullet}] \mapsto \sum_{i}(-1)^{i}\opn{rk}_A(C^{i})
  =\sum_{i}(-1)^{i}\opn{rk}_A H^{i}(C^{\bullet})$.
...
\begin{Rmk}
  ...
  for height functions (Banchoff's critial point theorem) \cite[Theorem 1]{MR225327}.
...
We explain the relationships between the local index and singularities.
...
The following K\"unneth type formula is important:
...
Let $(M,g)$ be a Riemannian manifold and $f$ be an analytic function on $M$.
...
\begin{Eg} Instead of Morse degree, we can define the local index for $C^{1}$-functions with at most isolated critical points.

  Let $f:\R^{2}\to \R; (x,y)\mapsto x^{2}+y^{2}$

\end{Eg}



$f^{-1}(\delta)\cap B_{\vep}^{n}$ is called a \ind{negative Milnor fiber}{negative Milnor fiber} of $f$.


\begin{Rmk}
  This index is a certain formalization of Poincar\'e-Hopf index $\opn{ind}_{\opn{PH}}(\opn{grad}(f),0)$
  of gradient field $\opn{grad}f$ ( or exact differential 1-form $df$) of smooth functions.
  Khimshiashvili's formula \footnote{Khimshiashvili is called Himshiashvili for some literatures}
  \begin{Thm}[Khimshiashvili's formula \cite{MR0458467}]
    Let $f$ be an analytic germ
    \begin{align}
      1-\chi_{\opn{top}} (f^{-1}(\delta)\cap B^{n}_{\vep})=\opn{sign}(-\delta)^{n}\opn{ind}_{\opn{PH}}(\opn{grad}f,0)
    \end{align}
    where $0<|\delta|\ll \vep \ll 1 $
  \end{Thm}
  In particular,
  \cite{} is a good survey.
\end{Rmk}
...
a preorderd set with an Alexandrov topology (c.f. \cite[]{}.).
...
We start from for tropical curves. Our theorem for tropical curve
...
\begin{Rmk}
  We would like to remark extremely polite smooth functions are \emph{not}
  a Morse function in the sense of \emph{stratified Morse theory}
  even though $f$ is a restriction of a Morse function.
  Every open atlas of tropical curve (or rational polyhedral space) can be considered
  as a smooth stratification satisfying Whitney's condition \cite[]{}.

  Therefore, we can consider a Morse function
  for the coarse polyhedral stratification.

  In our setting $df_v(Q)=0$ for every generalized tangent space
  at $v$ is important.

  We should add the isotopy condition for general cases:
...
In particular, we can construct a typical tropical curve
...
\begin{Rmk}
  ...
  From the existance of winning strategy of chip-firing game　\cite[Thereom 1.9]{MR2355607},
...
Every integral affine manifold is a tropical manifold, so we use
the theory of tropical homology.
...
\begin{Rmk}
  ...
  \begin{Eg}
    Thus, $c_{k}(B_0)$ should be $1$ except $k=0$, i.e., $\opn{td}(B_0)=1$. We already see $\hat{\mcal{A}}(X(B_0))=\opn{td}(X(B_0))=1$.
    From semi-flat SYZ mirror symmetry, the above conjecture should be true. In the case of tropical tori, see \ref{}.
  \end{Eg}
\end{Rmk}

\begin{Rmk}
  If $B_0$ is not an Hessian mmanifold, then we cannot apply the above proof
...
$(\opn{int}(\Delta^{[n]}),\check{D}d\check{f})$ are isometoric as
...
\begin{Rmk}
  ...
  over $\underline{\R}_{\opn{log}}$ and its diffenretial is
  commutaive with the moment map in the sense of .
\end{Rmk}
...
In our definition of Lagragian section and index, we do not define the higher operation
...
\subsection{Defnition of tropical manifolds}

In this section, we recall about polyhedral spaces, tropical manifolds and sheaves on them from  \cite{MR3903579,gross2019sheaftheoretic}.
...
\begin{Eg}[{tropical projective spaces}]


  We see tropical toric varieties in \cref{X}.
\end{Eg}
...
Then, the canonical monomorphism $\mcal{O}^{\times}_B \to \mcal{A}^{0,0}_B$
induces the following commutative diagram \footnote{Several similar diagrams apprear
  in literature}:
...
\end{align}
$\mcal{M}_{B}^{\times}$ is also acyclic, and thus $\opn{Pic}(B)\deq H^{1}(B,\mcal{O}^{\times}_B)\simeq  \opn{Im} (\delta : H^{0}(B,\mcal{D}iv_{B})\to H^{1}(B, \mcal{O}^{\times}_B))$ \cite[Lemma 4.5]{jellLefschetzTheoremTropical2018a}.

\begin{Def}[Smooth Cartier data]
  A \ind{smooth Cartier data}{smooth Cartier data} $s=\{f_i\}$ is an element of
  $H^{0}(B,\mcal{A}_B^{0,0}/\mcal{O}_{B}^{\times})
    \simeq H^{0}(B,\mcal{Z}^{1}_B/\mcal{F}_{\Z,B}^{1})$.
\end{Def}
...
$\mb{D}_{A_X}$ is called the \ind{dualizing complex}{dualizingcomplex} over $A$ of $X$.
...
\end{align}
In particular, $\mb{D}_{B}\simeq \mcal{D}(\Z_{B})\simeq \FBZ^{n}[n]$.
...
In this section,
we recall the notion of affine manifolds with singularities
following
...
An integral affine manifold is a special tropical manifold which is homeomorphic to a topological manifold.
\begin{Def}[integral affine manifold, Hessian manifold]
  ...
  An \ind{real Monge-Ampere manifold}{real Monge-Ampere manifold} is a Hessian manifold satisfying the
...
We can consider the sheaf theoretic definition of integral affine manifold as explained in \cite[2.1]{kontsevichAffineStructuresNonArchimedean2006a} and thus integral affine manifold is a special case of tropical manifold.

Integral affine manifolds naturally appear as a base space of Lagrangian torus fibration \cite{duistermaatGlobalActionangleCoordinates1980a}. In particular, the deformation space of special Lagrangian submanifolds of a Calabi-Yau $n$-fold with a nowhere vanishing holomorphic $n$-form has an integral affine structure with a Hessian metric by McLean's theorem \cite{MR1664890}.

Every integral affine manifold has the following canonical

The integral affine structure is a very strict condition.
...
if and only if its orientable double cover has a compatible special affine structure.

\footnote{We have not check the original source of Chern's conjecture
  and Markus conjecture yet. Our knowledge about these conjectures
  mainly comes from \cite{goldmanRadianceObstructionParallel1984a}
  and \cite{MR3665000}.
}

As mentioned in \cite{}, if a closed affine manifold has a Hessian form, then the Markus conjecture is true by an analogue of Calabi's conjecture for Hessian manifolds proved in \cite{MR714338}.

By \cite{MR714338}, every closed special Hessian manifold $B_0$ has a flat Riemannian metric $g$ such that its Levi-Civita connection is equal to affine connection of $B_0$. By Bieberbach theorem, every closed flat Riemannian manifold is covered by a Riemannian flat torus and thus every closed integral Hessian manifold is a unramified cover of a tropical torus as mentioned in \cite[5.2]{MR1882331}.

Every compact integral affine manifold has no infinite volume face, i.e., has a nontrivial finite volume.
If $B_0$ is a strongly integral affine manifold, we can define the set of lattice points $B_0(\Z)$ on  $B_0$ for a fixed strongly integral affine structure.

From now we recall the commutative diagram (\ref{eq: smoothcartier}) for integral affine manifolds.


We now see additional properties for sheaves on integral affine manifolds.

If $B_0$ is a integral affine manifold, $\mcal{O}^{\times}_{B_0}$ is a locally constant sheaf and the quotient sheaf $\mcal{W}^{1}_{\Z,B_0}$ is a $\Z$-local system on $B_0$.
...
\begin{Rmk}
  We write down why we say integral affine manifold is a certain analogue of complex manifold but not algebraic variety. Besides, there exists an integral affine manifold such that $h^{p,q}_{\opn{trop}}\ne h^{q,p}_{\opn{trop}}$. One of examples is a tropical analog of first Kodaira surface as explained in .
\end{Rmk}
...
If $B_0$ is a closed integral Hessian manifold, $X(B_0)$ and $\check{X}(B_0)$ has a K\"ahler structure.

Let $f:B_0' \to B_0$ be a finite unramified covering of $B_0$ such that $B_0'$ is a tropical torus.

Then, $X(B_0',K')$ (resp. $\check{X}(B_0',K')$)  is a complex torus and an unramified finite cover of $X(B_0,K)$ (resp. $\check{X}(B_0,K)$).
...
\begin{Rmk}[{Kodaira-Thurston surface}]
  ...
  and \cite[p.403]{MR1461965},
  $X(B_0)$ is a primary Kodaira surface and $X(B_0),\check{X}(B_0)$
  are diffeomorphic to Thurston's example of a symplectic but
  not K\"ahler manifold in \cite{MR402764} since all of them can
  be considered as a total space of $(\R/\Z)^{2}$-local system on
  a $2$-torus with the same monodromy representation.
...
This strong point gives a Floer theoretic approach of "sheaf cohomology theory" for tropical geometry without discussing homological algebra over idempotent semiring \footnote{In fact, in \cite[Example 6.5]{yoshitomi2011generators} the author give a simple example of divisors on tropical curves such that rank of divisor is different even though the $\mathbb{T}$-semimodule of the global section of tropical line bundle $\mcal{O}_C(D)$ is isomorphic. We will see this example later.}. The difficulty of homological algebra over idempotent semiring relates with the difficulty of higher dimensional Riemann-Roch theorem for tropical varieties.
...
An object of Fukaya category of $\check{X}$ is an oriented Lagrangian submanifold with additional data. The set of morphism is a graded vector space generated by a intersection point of two Lagrangian submanifolds when the two intersect transversely.
...
\footnote{Surprisingly, the words "sheaf" or "sheaves" is only at page 13 in \cite{} and there is no such words in }
...
\begin{Prop} If $B$ is an integral affine manifold and $A=\Lambda_{\opn{nov}}$ or $A=\Lambda_{\opn{nov}}^{\mb{K}}$, then

  $\opn{CF}^{\bullet}(L_1,L_2)$ is isomorphic to the classical Floer complex as a graded $\Lambda_{\opn{nov}}$-module.
\end{Prop}
...
\begin{Def}[Index for tropical curves]

\end{Def}

a smooth compact tropical curve $\overline{C}$.

Let $V(\overline{C})=\set{p\in \overline{C}\mid \opn{val}(p)=1}$. Then $\overline{C}_0\deq \overline{C}\setminus V(\overline{C})$ is a disjoint union of open interval.
Let $D$ be a divisor on $\overline{C}$.

such that $\opn{supp}D \cap V(C)=\emp$. In general, every divisor $D'$ on $C$ is equivalent to a such divisor $D$.

such like \cite[Lemma 3.4]{gathmannRiemannRochTheoremTropical2008a}.

When $D$ is an effective divisor on $\overline{C}$

We don't know more sophisticated interpretation and relationships between the above counting formula and tropical Riemann-Roch formula \cite{gathmannRiemannRochTheoremTropical2008a}.
...
\begin{Rmk}[A relation of arithmetic surfaces]
  ...
  It may seem that the definition $r_G(D)$ is far from the usual definition of the rank of linear system of divisor on algebraic curve, but the rank of classical linear system can be considered as the same definition of graph version, see \cite[Lemma 2.4]{MR2448666}.

\end{Rmk}

\section{General setting}

From this local index, we can define "the cardinality of lattices points of $s:M \to \check{X}_0(M)$" as follows.
...
Therefore, an numerical function
...
\begin{Eg}[An analog of Kodaira vanishing theorem]
  ...
  throgh codimension $1$ \cite[Theorem 3.3.5]{maclaganIntroductionTropicalGeometry2015a}
...
The following conjecture is a certain generalization of generalized Pick's formula for compact tropical curves and compact integral Hessian manifolds.

\begin{Conj}[{Floer theoretic intersection number v.s. algebraic geometrical intersection number}]
  Let $B$ be a compact smooth tropical manifold and $D$ a Cartier divisor on $B$. Then, there exists $s_D: B \to \check{X}_0(B)$ such that
  \begin{align}
    \chi(B,s_D)=\chi (\opn{CF}^{\bullet}(s_D))=\int_B \opn{ch}(D)\opn{td}(B).
  \end{align}
\end{Conj}
...
However, I have no idea now.

\begin{Rmk}
  There exists another tropical analogues of Poincare-Hopf theorem
...
There exists a acyclic resolution $\C_X \to \mcal{A}_X^{\bullet}$
...
If $X$ is a K\"ahker the filtration
...
This phenomina is different from analytic de Rham complexes of
complex manifolds.

More suprising phenomina is the existance of the monodromy map,
see \cite{}.
...
From now on, we assume $G$ is a dicrete group.
...
\begin{Def}[properly discontinuously]
  ...
  (ii) For each point $x\in X$, there exists an open neighboorhood
...
Our definition of `properly disvcontinuously' follows from
...
We can genelize for locally lattice polytopal complex by gluing
of them.
...
$P(\frac{1}{n}\Z)\deq \iota^{-1}_{n}(nP\cap M)$.
  ...
  BS-type extremely polite Lagragian section $s$.
...
\begin{Rmk}
  We can consider global version of the theory of valutitons of polytopal
...
\begin{Eg}[Lattice polyhedra of dimension $2$ revisit]
  ...
  we mension the following properties
\end{Eg}
...
\begin{Rmk}[Relationships for Eluer calculus and valuation]
  ...
  From Rota's theorem, $f$, $\check{f}$ are dual by Mobius inversion formula.
\end{Rmk}
